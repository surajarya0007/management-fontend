...
const ApiInventory: React.FC = () => {
  ...
  const handleSortChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSortDirection(
      sortKey === k && sortDirection === "ascending"
        ? "descending"
        : "ascending",
    );
    setSortKey(k);
  ...
  const handleDeleteApi = async (id: string) => {
    ...
    try {
      ...
      if (response.ok) {
        setApiList((prev) => prev.filter((api) => api._id !== id));
      } else {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`${API_BASE}/api/${editApi._id}`, {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          name: editApi.name,
          endpoint: editApi.endpoint,
          description: editApi.description,
          owner: editApi.owner,
          status: editApi.status,
          version: editApi.version,
          lastScanned: editApi.lastScanned,
        }),
      });
      if (response.ok) {
        const body = await response.json();
        const updated: Api = body.api ?? body;
        setApiList((prev) =>
          prev.map((a) => (a._id === updated._id ? updated : a)),
        );
        setEditMsg({ type: "success", text: "API updated successfully!" });
        setTimeout(() => {
          setEditApi(null);
          setEditMsg(null);
        }, 1200);
        ...
        setEditMsg({
          type: "error",
          text: "Failed to update API. Please try again.",
        });
      }
    } catch (error) {
  ...
  const handleAddApi = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmitMsg({
          type: "error",
          text: "Failed to add API. Please try again.",
        });
      }
    } catch (error) {
    ...
    <Layout>
          ...
          <form onSubmit={handleAddApi} className="p-5">
            ...
            >
              {[
                ...
                <motion.div key={field.key} variants={staggerItem}>
                  <input
                    ...
                    onChange={(e) =>
                      setNewApi({ ...newApi, [field.key]: e.target.value })
                    }
                    style={{ ...inputStyle }}
                    onFocus={fieldFocus}
                    onBlur={fieldBlur}
              ...
              <motion.div variants={staggerItem}>
                <select
                  value={newApi.status}
                  onChange={(e) =>
                    setNewApi({ ...newApi, status: e.target.value })
                  }
                  style={{ ...inputStyle, cursor: "pointer" }}
                  onFocus={fieldFocus}
                  onBlur={fieldBlur}
                >
                  <option value="Active" style={{ background: "#0a0d0d" }}>
                    Status: Active
                  </option>
                  <option value="Inactive" style={{ background: "#0a0d0d" }}>
                    Status: Inactive
                  </option>
                </select>
              </motion.div>
            ...
            <div className="flex items-center gap-3">
              <motion.button
                ...
                style={{
                  background: isSubmitting ? "#115e59" : "#0d9488",
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <option value="name" style={{ background: "#0a0d0d" }}>
                  Name
                </option>
                <option value="endpoint" style={{ background: "#0a0d0d" }}>
                  Endpoint
                </option>
                <option value="owner" style={{ background: "#0a0d0d" }}>
                  Owner
                </option>
                <option value="status" style={{ background: "#0a0d0d" }}>
                  Status
                </option>
                <option value="lastScanned" style={{ background: "#0a0d0d" }}>
                  Last Scanned
                </option>
              </select>
            </div>
                    ...
                    <td colSpan={6} className="px-5 py-12 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm" style={{ color: "#0d9488" }}>
                          No APIs found. Add your first API above.
                        </p>
                      </div>
                    </td>
                  </tr>
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5">
                        <span
                          ...
                          style={statusBadgeStyle(api.status)}
                        >
                          {api.status}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            style={{
                              color: "#f87171",
                              opacity: deletingId === api._id ? 0.5 : 1,
                              cursor:
                                deletingId === api._id
                                  ? "not-allowed"
                                  : "pointer",
                          ...
                          >
                            {deletingId === api._id ? "Deleting…" : "Delete"}
                          </button>
                        </div>
                      </td>
                    </motion.tr>
      ...
      <AnimatePresence>
        {editApi && (
          ...
          >
            <motion.div
              style={{ ...card, width: "100%", maxWidth: "560px" }}
            ...
            >
              <div
                ...
                style={{ borderBottom: "1px solid #1f2e2d" }}
              >
                <h3 className="text-sm font-semibold text-white">Edit API</h3>
                <button
                  onClick={() => {
                    setEditApi(null);
                    setEditMsg(null);
                  ...
                  style={{ color: "#5a7d78" }}
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLElement).style.color = "#f0fdfa";
                  }}
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.color = "#5a7d78";
                  }}
                >
                  <svg
                    className="w-4 h-4"
              ...
              <form onSubmit={handleEditSubmit} className="p-5 space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={key}>
                      ...
                      <input
                        type="text"
                        required={["name", "endpoint", "owner"].includes(key)}
                        value={
                          (editApi as unknown as Record<string, string>)[key] ??
                          ""
                        }
                        onChange={(e) =>
                          setEditApi({ ...editApi, [key]: e.target.value })
                        }
                        style={{ ...inputStyle }}
                        onFocus={fieldFocus}
                        onBlur={fieldBlur}
                  ...
                  <div>
                    ...
                    <select
                      value={editApi.status}
                      onChange={(e) =>
                        setEditApi({ ...editApi, status: e.target.value })
                      }
                      style={{ ...inputStyle, cursor: "pointer" }}
                      onFocus={fieldFocus}
                      onBlur={fieldBlur}
                ...
                <div className="flex items-center gap-3 pt-1">
                  <motion.button
                    ...
                    whileHover={isEditSubmitting ? {} : { scale: 1.02 }}
                    whileTap={isEditSubmitting ? {} : { scale: 0.97 }}
                    className="inline-flex items-center gap-2 text-white text-sm font-medium px-5 py-2.5 rounded-lg"
                    style={{
                      background: isEditSubmitting ? "#115e59" : "#0d9488",
